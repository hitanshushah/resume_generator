...
def generate_resume_stream(request):
    ...
    try:
        ...
        yield f"data: {json.dumps({'type': 'progress', 'total': total_sections, 'current': 0, 'message': 'Starting resume generation...'})}\n\n"
        ...
        for index, section_info in enumerate(sections, 1):
            ...
            try:
                ...
                yield f"data: {json.dumps({'type': 'section', 'section': section_name, 'title': section_title, 'content': section_response, 'progress': {'total': total_sections, 'current': index}})}\n\n"
